...
// Only iterate real arrays of rows; a missing/malformed option must not fatal.
$left_items  = is_array( $left_items ) ? $left_items : array();
$right_items = is_array( $right_items ) ? $right_items : array();
...
<?php foreach ( $left_items as $item ) : if ( ! is_array( $item ) ) { continue; }
...
<?php foreach ( $right_items as $item ) : if ( ! is_array( $item ) ) { continue; } ?>
            <a href="<?php echo esc_url( blue_raeven_nav_url( $item['url'] ) ); ?>"><?php echo esc_html( $item['label'] ); ?></a>
<?php endforeach; ?>
        </div>
